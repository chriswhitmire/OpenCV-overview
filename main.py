...
#########
# Doing edge detection on a video
#########

### first lets load in the video. Then, when its loaded, apply the canny
### algorithm to the loaded images
...
while capture.isOpened():
    ...
    if isLoaded == True:
        # Use the canny algorithm to find edges
        edgeImg = cv2.Canny(img, 80, 100)

        # show the current frame in a window titled "Webcam data"
        cv2.imshow("Webcam data", edgeImg)
        # if you press the q key or click on the exit button, then close the video window (& is bitwise "and")
        # This also makes it wait 25ms between each frame
        # Note: "getWindowProperty"
        if cv2.waitKey(25) & 0xFF == ord('q') or cv2.getWindowProperty("Webcam data", 0) < 0:
            break
